...
When('user fills and submits the Interstate Authority to Breed form', async () => {
  ...
  browser.wait(EC.elementToBeClickable(Racing.Interstate), 5000).then(function () {});
...
When('user proceeds with the payment option', async () => {
  ...
  elm.sendKeys('01');
  await browser.executeScript('window.scrollTo(0,5000);')
  browser.wait(EC.elementToBeClickable( Racing.ExpYr), 5000).then(function () {});
  Racing.ExpYr.sendKeys('30');
  browser.wait(EC.elementToBeClickable(Racing.SecCode), 5000).then(function () {});
...
When('user completed the documentation', async () => {
